...
:root{{
  ...
  --spy:#1d4ed8;--mag:#c2410c;--tsl:#b91c1c;--btc:#9333ea;
  --zone:{zone_color};
}}
...
<script>
const DATA = {payload};
const COLORS = {{SPY:'#1d4ed8',MAG7:'#c2410c',TSLA:'#b91c1c',BTC:'#9333ea'}};
...
function renderTable(){{
  ...
  rows.forEach(r=>{{
    ...
    const allocFmt = alloc>0&&!skip ? '$'+(Math.round(alloc)).toLocaleString('en-US') : '—';
  ...
  document.getElementById('residualAmt').textContent =
    '$'+Math.round(residual).toLocaleString('en-US') + ' → dry powder / PHP buffer';
}}
