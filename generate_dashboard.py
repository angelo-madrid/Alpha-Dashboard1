...
# ── FAIR VALUE LIVE DATA ──────────────────────────────────────────────────────
print("\nFetching fair value data via yfinance...")

FV_CONFIG = [
    # (yf_ticker, display, company, asset_class)
...
def _fetch_fv(yt):
    try:
        ...
        print(f"  {yt} info fetch failed: {e}")
        info = {}
    return info
...
def _build_card(yt, disp, co, cls):
    ...
    low  = info.get("targetLowPrice")  or info.get("fiftyTwoWeekLow")
    high = info.get("targetHighPrice") or info.get("fiftyTwoWeekHigh")
    fill_pct = 50.0
    if price and low and high and high > low:
        fill_pct = max(0.0, min(100.0, (price - low) / (high - low) * 100))
    bar_color = "#2563eb" if (upside or 0) >= 10 else "#16a34a" if (upside or 0) >= 0 else "#dc2626"
    bcls, btxt = _badge(info.get("recommendationKey"), upside)
    decimals = 0 if cls == "crypto" else 2

    if cls == "crypto":
        m = [("Market Cap", _fmt_money(info.get("marketCap"), 0)),
             ("52w High",   _fmt_money(info.get("fiftyTwoWeekHigh"), 0)),
             ("52w Low",    _fmt_money(info.get("fiftyTwoWeekLow"),  0))]
    elif cls == "etf":
        dy = info.get("yield") or info.get("dividendYield")
        m = [("Div Yield", f"{dy*100:.2f}%" if dy else "—"),
             ("52w High",  _fmt_money(info.get("fiftyTwoWeekHigh"), 2)),
             ("52w Low",   _fmt_money(info.get("fiftyTwoWeekLow"),  2))]
    else:
    ...
    target_txt = _fmt_money(target, decimals) if target else "—"
    price_txt  = _fmt_money(price,  decimals) if price else "—"
    low_txt    = _fmt_money(low,    decimals) if low else "—"
    high_txt   = _fmt_money(high,   decimals) if high else "—"
    print(f"  {disp}: price={price_txt} target={target_txt} upside={upside_txt}")
    return f"""
    <div class="fv-card">
      <div class="fv-head">
      ...
      <div class="fv-prow">
        ...
        <div class="fv-pblk"><div class="fv-plbl">TARGET</div><div class="fv-pval {upside_cls}">{target_txt}</div></div>
...
.fv-section{{margin-top:36px}}
.fv-title{{font-family:'DM Mono',monospace;font-size:11px;color:var(--mut);letter-spacing:1.4px;font-weight:500;margin-bottom:14px}}
.fv-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:12px}}
.fv-card{{background:var(--surf);border:.5px solid var(--bdr);border-radius:6px;padding:14px 16px}}
.fv-head{{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px}}
.fv-tk{{font-family:'DM Mono',monospace;font-size:16px;font-weight:600;color:var(--txt)}}
.fv-co{{font-family:ui-sans-serif,system-ui,sans-serif;font-size:11px;color:var(--mut);margin-top:2px}}
.fv-bdg{{font-family:'DM Mono',monospace;font-size:10px;padding:3px 8px;border-radius:4px;font-weight:500}}
.fv-buy{{background:#dcfce7;color:#166534}}
.fv-hold{{background:#fef3c7;color:#92400e}}
.fv-cau{{background:#fee2e2;color:#991b1b}}
.fv-prow{{display:flex;gap:14px;align-items:flex-end;margin-bottom:12px}}
.fv-pblk{{flex:1}}
.fv-plbl{{font-family:'DM Mono',monospace;font-size:9px;color:var(--mut);letter-spacing:.8px;margin-bottom:3px}}
.fv-pval{{font-family:'DM Mono',monospace;font-size:18px;font-weight:600;color:var(--txt)}}
.fv-pos{{color:#15803d}}
.fv-neg{{color:#b91c1c}}
.fv-bar{{margin:8px 0 6px}}
.fv-blbl{{display:flex;justify-content:space-between;font-family:'DM Mono',monospace;font-size:9px;color:var(--mut);margin-bottom:4px}}
.fv-bg{{height:5px;background:#f3f4f6;border-radius:3px;overflow:hidden;position:relative}}
.fv-fl{{height:100%;border-radius:3px}}
.fv-mtx{{display:flex;gap:8px;margin-top:10px}}
.fv-mbox{{flex:1;background:var(--dim);border-radius:4px;padding:6px 9px;text-align:center}}
.fv-mlbl{{font-family:'DM Mono',monospace;font-size:9px;color:var(--mut);margin-bottom:2px}}
.fv-mval{{font-family:'DM Mono',monospace;font-size:12px;font-weight:600;color:var(--txt)}}
.fv-disc{{font-family:'DM Mono',monospace;font-size:9px;color:var(--mut);margin-top:18px;padding-top:10px;border-top:.5px solid var(--bdr);line-height:1.6}}
...
<div class="page">
  ...
  <div class="fv-section">
    <div class="fv-title">FAIR VALUE ASSESSMENT · LIVE</div>
    <div class="fv-grid">{fv_cards_html}
    </div>
    <div class="fv-disc">Live yfinance data · Targets, P/E, growth, and recommendations sourced from analyst consensus aggregated by Yahoo Finance · Crypto and ETFs use 52-week range when analyst targets are unavailable · Informational only, not investment advice.</div>
  </div>
